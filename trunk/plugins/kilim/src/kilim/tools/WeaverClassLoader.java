 ...
 */
/**
 * Originally developed for JavaFlow, adapted for use in Kilim by Ted Stockwell.
 */
package kilim.tools;
...
/**
 * {@link URLClassLoader} with bytecode instrumentation for kilim.
 *
 * <p>
 ...
 * jar file.
 */
public final class WeaverClassLoader extends URLClassLoader {

    private final static Logger log = Logger.getLogger(WeaverClassLoader.class.getName());

    private final Weaver transformer;
     ...
     *      May not be null.
     */
    public WeaverClassLoader(URL[] urls, ClassLoader parent, Weaver transformer) {
    ...
    public WeaverClassLoader(URL[] urls, ClassLoader parent) {
        this(urls,parent,new Weaver());
    }
    public WeaverClassLoader(List<URL> urls, ClassLoader parent) {
        this(urls.toArray(new URL[urls.size()]),parent,new Weaver());
    }
    ...
    public Class<?> forceLoadClass(String classname)
        ...
        log.fine("force loading " + classname);
    ...
    protected synchronized Class<?> loadClass(String classname, boolean resolve)
        ...
        if (isParentFirst(classname)) {
            try {
                ...
                log.fine("Class " + classname + " loaded from parent loader "
                    + "(parentFirst)");
            } catch (ClassNotFoundException cnfe) {
                theClass = findClass(classname);
                log.fine("Class " + classname + " loaded from ant loader "
                    + "(parentFirst)");
            }
        } else {
            try {
    ...
    protected Class<?> defineClassFromData(final byte[] classData, final String classname) {
        return AccessController.doPrivileged(new PrivilegedAction<Class<?>>() {
            public Class<?> run() {
                ...
                ClassInfo classInfo= new ClassInfo(classname, classData);
                List<ClassInfo> classes= transformer.weave(classInfo);
                Class<?> result= null;
                final ProtectionDomain domain = this.getClass().getProtectionDomain();
                for (ClassInfo ci:classes) {
                	Class<?> c= defineClass(ci.className,ci.bytes,0,ci.bytes.length,domain);
                	if (ci.className.equals(classname)) {
                        result= c;
                	}
                }
                if (result == null)
                	result= defineClass(classname,classData,0,classData.length,domain);
                	//throw new InternalError("Weaver failed to produce a matching class");
                return result;
            }
        }, acc);
    }
