 ...
 *	this implementation handlers register themselves with a 'signal' 
...
public class Signals {
	...
	final public synchronized static void signal(final Identifier path) {
		...
		if (set != null) {
			HashSet<SignalHandler> actions= (HashSet<SignalHandler>) set.clone();
			for (final SignalHandler action:actions) {
				try {
					action.signal(path);
				}
				catch (Throwable t) {
					Logging.warning("Error thrown from signal handler", t);
				}
			}
		}
	}
